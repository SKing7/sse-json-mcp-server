...
class SSEFormatConverter {
   ...
   * @param {string} baseTimestamp - 基础时间戳
  ...
  static convertMultipleSSEEvents(multipleSSEData, baseTimestamp = null) {
    // 按双换行符分割多个事件
    const events = multipleSSEData.trim().split('\n\n');
    const result = [];
    let currentTimestamp = parseInt(baseTimestamp || Date.now().toString());

    for (const eventData of events) {
      if (eventData.trim()) {
        const converted = this.convertSSEData(eventData + '\n\n', currentTimestamp.toString());
        result.push(...converted);
        currentTimestamp += Math.random() * 1000 + 100; // 增加随机间隔
      }
    }
